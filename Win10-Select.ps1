...
$dotScript = "WinREProvPackage.ps1"
if (Test-Path $dotScript) {
    Write-Host  -ForegroundColor Magenta "Dot-Sourcing functions from $dotScript "
    . $dotScript
    $ProvPackages = Select-OSDCloudProvPackage
}

#Start OSDCloud ZTI the RIGHT way
Write-Host  -ForegroundColor Cyan "Start OSDCloud with MY Parameters"
Start-OSDCloud -OSLanguage de-de

#Anything I want  can go right here and I can change it at any time since it is in the Cloud!!!!!
Write-Host  -ForegroundColor Cyan "Starting OSDCloud PostAction ..."

if ($ProvPackages) {
    $ProvPackages | Add-OSDCloudProvPackage
}
